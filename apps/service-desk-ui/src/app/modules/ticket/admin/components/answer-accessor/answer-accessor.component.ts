...
export class AnswerAccessorComponent implements ControlValueAccessor, AfterViewChecked {
  ...
  link(): void {
    ...
    this.writeValue(
      this.markdown.format(formatter, `[term](https://адрес_ссылки)`, { defaultText: 'описание ссылки' })
    );
  }
  ...
  image(): void {
    ...
    this.writeValue(
      this.markdown.format(formatter, `![term](https://адрес_ссылки)`, { defaultText: 'описание изображения' })
    );
  }
  ...
  quote(): void {
    ...
    this.writeValue(
      this.markdown.format(formatter, `> term`, { defaultText: 'цитата', multilineTemplate: '\n\n> term\n\n' })
    );
  }
...
~~~`;
    ...
    this.writeValue(
      this.markdown.format(formatter, template, { defaultText: 'фрагмент', multilineTemplate: '`term`' })
    );
  }
  ...
  number_list(): void {
    ...
    this.writeValue(
      this.markdown.format(formatter, template, { defaultText: 'Элемент списка', multilineTemplate: `\n${template}` })
    );
  }
