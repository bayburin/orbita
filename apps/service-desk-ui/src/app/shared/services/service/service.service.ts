...
export class ServiceService implements BreadcrumbServiceI {
   ...
   */
  loadService(serviceId: number, caching = false): Observable<Service> {
    this.loadServiceUri = `${environment.serverUrl}/api/v1/services/${serviceId}`;
    if (caching) {
  ...
  getNodeName(): Observable<string> {
    return this.service$.asObservable().pipe(map((service: Service) => (service ? service.name : '')));
  }

  getParentNodeName(): Observable<string> {
    return this.service$.asObservable().pipe(map((service: Service) => (service ? service.category.name : '')));
  }
