...
describe('ServiceService', () => {
  ...
  describe('#loadService', () => {
    ...
    beforeEach(() => {
      loadServiceUri = `${environment.serverUrl}/api/v1/services/${service.id}`;
    ...
    it('should return Observable with Service data', () => {
      serviceService.loadService(service.id).subscribe((data) => {
    ...
    describe('with "caching" flag', () => {
      it('should set into "returnCached" attribute true value', () => {
        serviceService.loadService(service.id, true).subscribe((data) => {
          expect(data).toEqual(service);
    ...
    describe('with "returnCache" attribute', () => {
      beforeEach(() => {
        serviceService.loadService(service.id, true).subscribe();
        httpTestingController
          .expectOne({
            method: 'GET',
            url: loadServiceUri,
      ...
      it('should set into "returnCached" attribute false value', () => {
        serviceService.loadService(service.id).subscribe(() => {
  ...
  describe('Operations with tickets', () => {
    ...
    describe('#replaceQuestions', () => {
      ...
      beforeEach(() => {
        ...
        question = TicketFactory.create(TicketTypes.QUESTION, {
          id: ticket.id,
          ticket: { name: 'Тестовый вопрос. Новая редакция' },
        });
        correction = TicketFactory.create(TicketTypes.QUESTION, {
          id: 2,
          ticket: { name: 'Тестовый вопрос. Старая редакция.' },
        });
    ...
    describe('#removeDraftTickets', () => {
      ...
      beforeEach(() => {
        draftTicket = TicketFactory.create(TicketTypes.QUESTION, {
          id: 3,
          ticket: { state: 'draft', name: 'Тестовый вопрос 3' },
        });
