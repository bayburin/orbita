...
/**
 * Обрабатывает фильтры, связанные с таблицей заявок
 *
...
export function processSdRequestTableFilters(filters: PrimeFilter) {
  ...
  // Для фильтров таблицы по статусу нужен целиком объект, а для сервера только строка статуса. Поэтому для хранилища
  // меняем объект статуса на сам статус. Этот статус будет использоваться при запросах на сервер.
  if (filters.status && filters.status.value) {
    filters.status.value = filters.status.value.status;
  }
  // Для фильтров таблицы по приоритету нужен целиком объект, а для сервера только строка приоритета. Поэтому для
  // хранилища меняем объект приоритета на сам приоритет. Этот приоритет будет использоваться при запросах на сервер.
