...
export function processSvtItemTableFilters(filters: PrimeFilter) {
  // Для фильтров таблицы по типу ВТ нужен целиком объект, а для сервера только ID типа. Поэтому для хранилища меняем объект типа на сам тип. Этот тип будет использоваться при запросах на сервер.
